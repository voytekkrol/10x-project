...
describe('useGenerateFlashcards', () => {
  ...
  it('should load draft text on mount if available', () => {
    ...
    (loadDraft as any).mockReturnValueOnce(longDraftText);
  ...
  it('should update source text with validation', async () => {
    ...
    expect(result.current.sourceText.isValid).toBe(false);
    expect(result.current.sourceText.validationError).toBe('Source text must be at least 1000 characters (currently 5)');
  ...
  it('should handle generate proposals successfully', async () => {
    ...
    (generateProposals as any).mockResolvedValueOnce({
      id: 'gen-123',
  ...
  it('should handle rate limit errors', async () => {
    ...
    (generateProposals as any).mockRejectedValueOnce(rateLimitError);
